...
Future<Iterable<CallLogEntry>> getCallLogs(){
  ...
  getTitle(CallLogEntry entry){
    ...
    if(entry.name == null) {
      ...
      case CallType.outgoing:
        return Text('${entry.number}',style: TextStyle(color: Colors.greenAccent,fontSize: 20),);
      case CallType.missed:
       return Text('${entry.number}',style: TextStyle(color: Colors.red[400],fontSize: 20));
      default:
        return Text('${entry.number}',style: TextStyle(color: Colors.indigo[700],fontSize: 20));
    }
    }
 ...
 switch(entry.callType){
      ...
      switch(entry.callType){
      case CallType.outgoing:
        return Text('${entry.name}',style: TextStyle(color: Colors.greenAccent,fontSize: 20),);
      case CallType.missed:
       return Text('${entry.name}',style: TextStyle(color: Colors.red[400],fontSize: 20));
      default:
        return Text('${entry.name}',style: TextStyle(color: Colors.indigo[700],fontSize: 20));
    }
    }
